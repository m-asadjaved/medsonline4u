...
export default function ProductActions({ productId, variations, product }) {
	...
	const handleAddToCart = () => {
		if (!selectedVariation)
			return alert("Please select a variation first.");
		addToCart(productId, selectedVariation.id, quantity);
    if (typeof window !== "undefined") {
      window.gtag("event", "add_to_cart", {
        currency: "USD",
        value: selectedVariation.variation_price,
        items: [
          {
            item_id: product.id,
            ...
            quantity: quantity,
            price: selectedVariation.variation_price
          }
        ]
      });
    }
	...
	const displayMrp =
		selectedVariation &&
		parseFloat(selectedVariation.variation_mrp).toFixed(2);
