...
export default function ProductEditor({ initial = SAMPLE_PRODUCT }) {
	...
	async function fetchProductData() {
		...
		try {
			...
			// Use helper to make sure images is an array of url strings
			const normalizedImages = normalizeImagesFromServer(data.images);

			// Keep product.images as primitive url strings for saving convenience
			data.images = normalizedImages;

			// UI uses objects with { name, url }
			const uiImages = normalizedImages.map((u, i) => ({
				name: `remote-${i}`,
				url: u,
			}));
