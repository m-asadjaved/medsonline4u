...
export default function ShopPage() {
  ...
  async function fetchProducts() {
    setLoading(true);
    try {
      const res = await fetch(`/api/products/search/${currentPage}`);
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <section className="md:col-span-9 lg:col-span-9">
          <div className="bg-white p-4 rounded-xl shadow-sm">
            <div className="text-sm text-slate-600">{totalProducts} products found</div>
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {!loading && products.map(p => (
                <article key={p.id} className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
                  <Link href={`/products/${p.id}`}><Image src={p.image_url} alt={p.name} className="h-40 w-full object-cover" width={500} height={500} />
